...
def sync_mtproxy_service():
    """Rebuild MTProxy service with one unique secret per active device."""
    if os.geteuid() != 0:
    ...
    if not binary.exists() or not proxy_secret.exists() or not proxy_config.exists():
        raise RuntimeError("MTProxy не найден. Сначала запустите установщик MTProxy")
